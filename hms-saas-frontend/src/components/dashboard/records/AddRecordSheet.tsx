...
import { addRecord } from "@/app/actions/record-actions";
...
export function AddRecordSheet() {
 ...
 const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  if (result.success) {
    ...
    alert("Something went wrong saving the record!");
  }
 };
      ...
      <SheetContent className="w-full sm:max-w-[480px] overflow-y-auto border-l border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-2xl p-6 sm:p-8">
        <SheetHeader className="mb-8">
          ...
          <SheetDescription className="text-sm text-zinc-500 dark:text-zinc-400 font-dmsans">
            Log a new diagnosis and issue prescriptions for a patient.
          </SheetDescription>
        </SheetHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label htmlFor="notes" className={labelClass}>Clinical Notes</label>
            <textarea 
              id="notes" 
              name="notes" 
              rows={4}
              placeholder="Patient symptoms, recovery plan, etc." 
